...
func testWorkspace(t *testing.T, entries []catalog.Entry, files map[string]string) *workspace.Workspace {
	...
	for rel, content := range files {
		...
		if strings.HasPrefix(rel, "annotations-cache/meta/") {
			...
			os.WriteFile(filepath.Join(dir, "annotations-cache", "meta", ".format"), []byte(harvest.MetaStamp()), 0o644)
		}
	}
...
`)
	...
	want := map[string]string{
		...
		"Grit/fx/Flute Riser.wav":                "FX/_Unsorted/Grit/Flute Riser.wav",
...
`)
	...
	// A flat family under a template with no {family}: the family name

...
func fakeSSH(t *testing.T, extra string) (primeLog, masterMarker string) {
	...
	if runtime.GOOS == "windows" {
		// The stand-in is a sh script (no shebang support on Windows), and
		// the behavior under test — ControlMaster priming/multiplexing — is
		// disabled there because Win32-OpenSSH doesn't implement it.
		t.Skip("ssh multiplexing tests are Unix-only")
	}
	dir := t.TempDir()
